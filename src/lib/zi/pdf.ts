...
export async function generarFacturaPDF(venta: Venta, cfg: ZIConfig) {
  ...
  const estimatedHeight = 158 + estimatedProductLines * 5 + (venta.observaciones ? 12 : 0) + (venta.tipo === "credito" ? 11 : 0) + (venta.tipo === "tradein" ? 15 : 0);
  const receiptHeight = Math.max(185, Math.min(340, estimatedHeight));
  ...
  if (venta.cliente?.nombre) {
    row("Cliente", venta.cliente.nombre.toUpperCase(), 8.5, true);
