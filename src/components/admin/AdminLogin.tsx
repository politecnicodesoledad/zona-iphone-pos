...
import { Lock, User, ShieldCheck, Headset } from "lucide-react";

export function AdminLogin() {
  const { login } = useSession();
  const [cfg] = useConfig();
  const [rol, setRol] = useState<"admin" | "asesor">("admin");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setErr("");
    try {
      const res = await login(identificador.trim(), pw);
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-[var(--mist)]">
      ...
      <div className="relative w-full max-w-sm bg-white rounded-3xl p-8 shadow-soft border border-[var(--line)]">
        <div className="flex items-center gap-2">
          <span className="zi-chip"><ShieldCheck className="w-3 h-3" /> {rol === "admin" ? "Panel administrativo" : "Panel de asesor"}</span>
        </div>
        ...
        {/* Selector de rol: solo cambia cómo se ve esta pantalla (título e
            ícono) — quién eres de verdad lo decide tu cuenta, no este botón.
            Ayuda a que cada quien confirme que está en el lugar correcto
            antes de escribir su contraseña. */}
        <div className="mt-6 grid grid-cols-2 gap-2 p-1 bg-[var(--mist)] rounded-xl">
          <button type="button" onClick={() => setRol("admin")}
                  className={`flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-bold uppercase tracking-wide transition ${rol === "admin" ? "bg-white shadow-sm text-[var(--ink)]" : "text-gray-400"}`}>
            <ShieldCheck className="w-3.5 h-3.5" /> Admin
          </button>
          <button type="button" onClick={() => setRol("asesor")}
                  className={`flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-bold uppercase tracking-wide transition ${rol === "asesor" ? "bg-white shadow-sm text-[var(--ink)]" : "text-gray-400"}`}>
            <Headset className="w-3.5 h-3.5" /> Asesor
          </button>
        </div>

        <form onSubmit={submit} className="mt-5 space-y-3">
          <label className="block">
            <span className="text-[10px] uppercase tracking-[0.15em] text-gray-500 font-semibold">Correo o nombre y apellido</span>
            <div className="mt-1.5 relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input
                type="text" value={identificador} autoFocus required
                onChange={(e) => { setIdentificador(e.target.value); setErr(""); }}
                placeholder="tu@correo.com  ó  Carlos Pérez"
          ...
          <button type="submit" disabled={busy} className="zi-btn-gold w-full text-sm disabled:opacity-60">
            {busy ? "Entrando..." : `Entrar como ${rol === "admin" ? "Admin" : "Asesor"}`}
          </button>
        </form>
      </div>
    </div>
